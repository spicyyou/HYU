...
void *thread_basic(void *arg)
{
  ...
  //printf(1, "%d thread_basic\n",sum++);
  if (val == 1) {
 //printf(1, "%d thread_basic before sleep\n",sum++);
  
    sleep(200);
	// printf(1, "%d thread_basic after sleep\n",sum++);
...
void join_all(int n)
{
 ...
 //printf(1, "%d join_all\n",sum++);
		//  printf(1,"expected %d\n",expected[i]);
// int sum = thread_join(thread[i],(void**)&retval); 
// printf(1,"sum : %d\n",sum);   
 if (thread_join(thread[i],(void**)&retval)) {
